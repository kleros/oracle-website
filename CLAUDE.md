...
- **Deck controller**: the single `<script>` at the bottom defines a `Deck` class (vanilla JS, no framework — the `componentDidMount` naming is just a convention). It handles wheel/key/touch/click navigation, always opens on slide 1, persists only the theme to `localStorage` (`kleros-oracle-theme`), and lets slides taller than the viewport scroll internally before advancing (`canScrollSlide`).
